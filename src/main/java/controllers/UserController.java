...
public class UserController extends AbstractController {
	...
	public ModelAndView loginMake(@Valid UserAccount userAccount, BindingResult bindingResult,
		...
		if (bindingResult.hasErrors()) {
			...
			response = objectMapper.readValue(new URL("http://autha.agoraus1.egc.duckdns.org/api/index.php?method=checkToken&token=" + tokenToVerify),
					Token.class);
			...
			if (response.isValid()) {
				...
				try {
					...
					usuario = objectMapper.readValue(new URL("http://autha.agoraus1.egc.duckdns.org/api/index.php?method=getUser&user=" + userAccount.getUsername()),
							User.class);
